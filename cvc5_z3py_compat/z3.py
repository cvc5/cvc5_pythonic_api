...
def fpToSBV(rm, x, s, ctx=None):
    ...
    ctx = _get_ctx(ctx)
    return BitVecRef(ctx.solver.mkTerm(ctx.solver.mkOp(Kind.FLOATINGPOINT_TOSBV, s.size()), rm.ast, x.ast), ctx)
...
def fpToUBV(rm, x, s, ctx=None):
    ...
    if debugging():
        ...
        _assert(is_bv_sort(s), "Third argument must be SMT bit-vector sort")
    ctx = _get_ctx(ctx)
    return BitVecRef(ctx.solver.mkTerm(ctx.solver.mkOp(Kind.FLOATINGPOINT_TOUBV, s.size()), rm.ast, x.ast), ctx)
